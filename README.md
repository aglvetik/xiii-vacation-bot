...
- Постоянная таблица активных отпусков в отдельном канале.
...
ACTIVE_VACATIONS_CHANNEL_ID=1501256029399285810
...
5. обновляет постоянную таблицу активных отпусков или создаёт её;
6. сразу проверяет просроченные отпуска;
7. запускает воркеры завершения и обновления таблицы каждые 60 секунд.

## Таблица активных отпусков

Бот поддерживает одно постоянное сообщение со списком активных отпусков в канале `1501256029399285810`. Сообщение обновляется при запуске, после одобрения отпуска, после досрочного завершения, после автоматического завершения и каждые 60 секунд.

В таблице показываются только видимые активные отпуска: запись в базе должна быть `ACTIVE`, участник должен быть на сервере и у него должна быть роль отпуска. Упоминания пользователей кликабельны, но не отправляют уведомления.

## Slash-команды

- `/vacations` — отключена и только показывает ссылку на канал с постоянной таблицей активных отпусков.

Команда отвечает только эфемерным сообщением и не загружает список из базы.

## Ручные изменения роли

- Если роль отпуска вручную снята с участника, отпуск остаётся активным внутри базы до исходного срока, но скрывается из постоянной таблицы активных отпусков.
- Если роль отпуска вручную вернуть до истечения отпуска, тот же отпуск снова появится в таблице с исходными датами и причиной.
...
Панель подачи заявок хранится в `bot_state.panel_message_id`. Таблица активных отпусков хранится в `bot_state.active_vacations_message_id`. Если одно из этих сообщений удалили, бот создаст новое при следующем обновлении.
